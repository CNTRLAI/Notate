...
import { geminiAgent } from "../agentLayer/geminiAgent";
...
export async function GeminiProvider(
  ...
  const apiKey = await getApiKey(activeUser.id, "gemini");
  if (!apiKey) {
    throw new Error("Gemini API key not found for the active user");
  }
  await initializeGemini(apiKey.key);
  ...
  if (
    ...
    const { content, webSearchResult: webSearchResultFromAgent } =
      await geminiAgent(genAI, messages, maxOutputTokens, userSettings, signal);

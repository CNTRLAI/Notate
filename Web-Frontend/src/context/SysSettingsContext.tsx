...
import { SystemSpecs } from "@/src/types/sysSpecs";
...
const SysSettingsProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchLocalModels = async () => {
    try {
      ...
      setLocalModels(
        Array.isArray(mockData.models)
          ? mockData.models.map((model: string | Model) => ({
  ...
  const handleRunOllama = async (model: string, activeUser: User) => {
    console.log("handleRunOllama", model, activeUser);
    if (!model) {
      toast({
        title: "Error",
